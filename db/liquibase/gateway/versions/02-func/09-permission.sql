...
CREATE OR REPLACE FUNCTION iud_permission() RETURNS trigger AS $$
  BEGIN
    IF TG_OP = 'INSERT' THEN
      ...
      SELECT NEW."roleId", NEW."routeId", op_id, NEW.fields::text[], NEW.scopes::text[], NEW."creatorId", NEW."creatorName"
    ...
    ELSIF TG_OP = 'UPDATE' THEN
      ...
      INSERT INTO permission ("roleId", "routeId", "operationId", fields, scopes, "updaterId", "updaterName")
      SELECT NEW."roleId", NEW."routeId", op_id, NEW.fields::text[], NEW.scopes::text[], NEW."updaterId", NEW."updaterName"
      FROM unnest(NEW."operationId") AS op_id;
      RETURN NEW;
