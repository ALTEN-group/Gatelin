...
-- method IDs from seeded data: 1=GET 2=POST 3=PUT 4=PATCH 5=DELETE 6=HEAD 7=OPTIONS
INSERT INTO routes ("resourceId", pattern, name, description, "isProtected", locked, operations, "methodIds", "creatorId", "creatorName") VALUES
  ...
  ((SELECT id FROM resource WHERE name='products' AND "serviceId"=(SELECT id FROM "service" WHERE name='ms-product')),
   '/search',  'searchProducts', 'Search products',  true,  false, ARRAY[2],  ARRAY[2,7], -1, 'system'),
  ((SELECT id FROM resource WHERE name='products' AND "serviceId"=(SELECT id FROM "service" WHERE name='ms-product')),
   '',         'addProduct',     'Add a product',    true,  false, ARRAY[6],  ARRAY[2,7], -1, 'system'),
  ((SELECT id FROM resource WHERE name='products' AND "serviceId"=(SELECT id FROM "service" WHERE name='ms-product')),
   '',         'updateProduct',  'Update a product', true,  false, ARRAY[4],  ARRAY[3,7], -1, 'system'),
  ((SELECT id FROM resource WHERE name='products' AND "serviceId"=(SELECT id FROM "service" WHERE name='ms-product')),
   '',         'deleteProducts', 'Delete products',  true,  false, ARRAY[11], ARRAY[5,7], -1, 'system'),
  -- categories resource
  ((SELECT id FROM resource WHERE name='categories' AND "serviceId"=(SELECT id FROM "service" WHERE name='ms-product')),
   '/search',  'searchCategories', 'Search categories',  true,  false, ARRAY[2],  ARRAY[2,7], -1, 'system'),
  ((SELECT id FROM resource WHERE name='categories' AND "serviceId"=(SELECT id FROM "service" WHERE name='ms-product')),
   '',         'addCategory',      'Add a category',     true,  false, ARRAY[6],  ARRAY[2,7], -1, 'system'),
  ((SELECT id FROM resource WHERE name='categories' AND "serviceId"=(SELECT id FROM "service" WHERE name='ms-product')),
   '',         'updateCategory',   'Update a category',  true,  false, ARRAY[4],  ARRAY[3,7], -1, 'system'),
  ((SELECT id FROM resource WHERE name='categories' AND "serviceId"=(SELECT id FROM "service" WHERE name='ms-product')),
   '',         'deleteCategories', 'Delete categories',  true,  false, ARRAY[11], ARRAY[5,7], -1, 'system')
;
