...
import { CONTROL_TYPES, ID_CONFIG, INPUT_TYPES, maxlength, min, minlength, required, } from "@dwtechs/crud-builder";
const ROLE_STYLE_ID = "role-chip-styles";
function injectRoleStyles(roles) {
    ...
    const chipRules = roles
        .filter((r) => r.color)
        .map((r) => `.role-color-${r.id}{background-color:${r.color}!important;color:#fff!important;display:block!important;margin-bottom:2px;}`)
        .join("");
...
export const CONSUMER_COLUMNS = ({ data }) => {
    const activeRoles = data.roles.filter((role) => !role.archived);
...
//# sourceMappingURL=consumer.conf.js.map