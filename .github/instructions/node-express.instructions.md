...
- `src/app.js`: builds the Express app, registers global middleware **before** routes, registers routes, then `errorHandler(app)` from `@dwtechs/errandler-express` **after** routes. Ends with `export default app;` — no cache init, no cron jobs, no `listen()`.
