...
describe("getTemplatesAndUserPreferences", () => {
  let getTemplatesAndUserPreferences;

  beforeAll(async () => {
    const module = await import("../../src/services/preference.js");
    getTemplatesAndUserPreferences = module.getTemplatesAndUserPreferences;
  });
  ...
  it("should query the preferences view for system templates and user preferences with (userId, resource)", async () => {
    const rows = [
      { id: 1, name: "Default", conf: {}, locked: true },
      { id: 5, name: "Custom", conf: {}, locked: false },
    ];
    execute.mockResolvedValue({ rows });

    const result = await getTemplatesAndUserPreferences(11, "dashboard");
    ...
    expect(client).toBeNull();
    expect(query).toEqual(expect.stringContaining("FROM preferences"));
    expect(query).toEqual(expect.stringContaining('locked OR "userId" = $1'));
    expect(result).toBe(rows);
  });
  ...
  it("should propagate errors from the query", async () => {
    ...
    await expect(
      getTemplatesAndUserPreferences(11, "dashboard"),
    ).rejects.toThrow(err);
  });
});
