...
describe('checkRoute middleware', () => {
  ...
  it('should set res.locals.route and call next() when route is found', () => {
    ...
    expect(log.debug).toHaveBeenCalledWith('checkRoute(url: /api/users, method: GET)');
    expect(log.debug).toHaveBeenCalledWith(`checkRoute(Route: ${JSON.stringify(mockRoute)})`);
  ...
  it('should pass method and url to routeSvc.getOne', () => {
    ...
    req.method = 'PUT';
    const mockRoute = { id: 4, url: '/api/users/:id/profile', isProtected: true };
    routeSvc.getOne.mockReturnValue(mockRoute);

    checkRoute(req, res, next);

    expect(routeSvc.getOne).toHaveBeenCalledWith('/api/users/123/profile', 'PUT');
    expect(log.debug).toHaveBeenCalledWith('checkRoute(url: /api/users/123/profile, method: PUT)');
    expect(res.locals.route).toEqual(mockRoute);
    expect(next).toHaveBeenCalledWith();
  });
  ...
  it('should handle URL with query parameters', () => {
    ...
    expect(routeSvc.getOne).toHaveBeenCalledWith('/api/users?limit=10&offset=0', 'GET');
    expect(res.locals.route).toEqual(mockRoute);
  });
});
