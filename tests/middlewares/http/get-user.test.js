...
describe('getUserByEmail middleware', () => {
  ...
  beforeAll(async () => {
    const httpModule = require('../../../src/utils/http.js');
  ...
  it('should populate req.body.rows and res.locals.user on success', async () => {
    const user = { id: 'u1', nickname: 'alice', roles: [1, 2], active: true, email: 'test@example.com' };
    ...
    expect(req.body.rows).toEqual([{ userId: 'u1', nickname: 'alice', roles: [1, 2] }]);
