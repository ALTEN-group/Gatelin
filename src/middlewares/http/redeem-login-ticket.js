...
import { loginTicketUrl } from "../../conf/pwd.js";
...
export function redeemLoginTicket(req, res, next) {
  ...
  if (!ticket) return next({ statusCode: 400, message: "Missing ticket" });

  if (!loginTicketUrl)
    return next({
      statusCode: 501,
      message: "Login resume is not configured",
    });

  http
    .query("POST", loginTicketUrl, undefined, { ticket })
    .then((r) => {
