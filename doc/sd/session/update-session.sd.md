...
sequenceDiagram
  ...
  f->>msg: put(/sessions) {} + Cookie: refreshToken, csrfToken<br/>X-CSRF-Token header (read from csrfToken cookie)<br/>(no access token required)
  deactivate f
  activate msg
  rect rgb(100, 200, 100, 0.2)
    note over msg: Toker-express Library Block<br/>Inputs:<br/>- refreshToken from req.body, falling back to refreshToken cookie when body value is absent<br/>- TOKEN_SECRET (env)<br/>Validates JWT signature AND expiration, extracts issuer (user id)
    msg--)msg: Decode refresh token (validates signature and expiration)
  end
  rect rgb(150, 50, 50, 0.5)
    break when refresh token is missing, invalid or expired
  ...
  activate msg
  msg--)msg: Get consumer from cache by refreshToken value
  rect rgb(150, 50, 50, 0.5)
  ...
  activate msg
  msg--)msg: Compare presented refreshToken to consumer's refreshToken (timing-safe)
  rect rgb(150, 50, 50, 0.5)
    break when refreshToken does not match
      msg--)msg: log 401 refresh token mismatch
      msg->>f: 401 refresh token mismatch
      deactivate msg
      activate f
      f->>u: Go to login page
      deactivate f
  ...
  rect rgb(100, 200, 100, 0.2)
    note over msg: Toker-express Library Block<br/>Inputs:<br/>- issuer (user id) from decoded refreshToken<br/>- consumer data: { nickname, roles }<br/>- TOKEN_SECRET (env)<br/>- ACCESS_TOKEN_DURATION (env)<br/>- REFRESH_TOKEN_DURATION (env)<br/>Generates new JWT tokens with updated expiration
  ...
  deactivate db
  activate msg
  msg--)msg: Update consumer in cache (accessToken, id and refreshToken indexes) with new tokens and roles
  rect rgb(100, 200, 100, 0.2)
